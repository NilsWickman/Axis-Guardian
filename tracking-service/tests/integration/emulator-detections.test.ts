...
describe('Detection Statistics', () => {
  ...
  it('calculates track counts correctly', () => {
    ...
    // Track 1: confirmed (3+ detections)
    for (let i = 0; i < 3; i++) {
      mockTime += 33
    ...
    expect(trackManager.getActiveTrackCount()).toBe(1) // Confirmed
    expect(trackManager.getPendingTrackCount()).toBe(2) // Pending
