"""
Pre-processed Detection Support Module

This module provides functionality to load and use pre-processed detections
from JSON files generated by the pre-processing script.
...
Usage:
    ...
    detections_map = load_preprocessed_detections("path/to/video.detections.json")
...
def load_preprocessed_detections(json_path: str) -> Optional[Dict[int, List[Any]]]:
    """
    Load pre-processed detections from file (auto-detects format).
    ...
    try:
        ...
        if actual_path is None:
            logger.warning(f"Pre-processed detections file not found: {json_path}")
            return None

        logger.info(f"Loading pre-processed detections from: {actual_path.name}")
        ...
        logger.error(f"Failed to load pre-processed detections: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def get_detection_for_frame(
    ...
    Args:
        detections_map: Dictionary from load_preprocessed_detections()
...
def find_detection_json_for_rtsp(rtsp_url: str, camera_id: str, quality: Optional[str] = None) -> Optional[str]:
    """
    Auto-detect the pre-processed detections JSON file for an RTSP stream.

    This function attempts to find the corresponding .detections.json file
    for a camera by looking in the preprocessed videos directory with quality folders.

    Args:
        rtsp_url: RTSP stream URL (e.g., "rtsp://localhost:8554/camera1")
        camera_id: Camera identifier (e.g., "camera1")
        quality: Quality preset to search for (e.g., "720p"). If None, searches all qualities.
    ...
    # Get quality from environment or use parameter
    if quality is None:
        quality = os.getenv('PREPROCESSED_QUALITY', os.getenv('RENDER_RESOLUTION', '720p'))

    # Map camera IDs to expected preprocessed video names (updated naming)
    camera_video_mapping = {
        'camera1': 'view-HC3-preprocessed',
        'camera2': 'view-HC4-preprocessed',
        'camera3': 'view-IP2-preprocessed',
        'camera4': 'view-IP5-preprocessed',
    }
    ...
    preprocessed_dir = project_root / "shared" / "cameras" / "preprocessed"

    # Try specified quality first
    quality_dir = preprocessed_dir / quality
    detections_path = quality_dir / f"{video_name}.detections.json"

    if detections_path.exists():
        logger.info(f"✓ Found pre-processed detections for {camera_id} at {quality}")
        return str(detections_path)

    # Fallback: try all quality folders (highest first)
    quality_order = ['4k', '1440p', '1080p', '720p', '480p', '360p']
    for fallback_quality in quality_order:
        fallback_dir = preprocessed_dir / fallback_quality
        fallback_path = fallback_dir / f"{video_name}.detections.json"

        if fallback_path.exists():
            logger.info(f"✓ Found pre-processed detections for {camera_id} at {fallback_quality} (fallback)")
            return str(fallback_path)

    # Also check old rendered/ location for backward compatibility
    rendered_dir = project_root / "shared" / "cameras" / "rendered"
    legacy_path = rendered_dir / f"{video_name.replace('preprocessed', 'rendered')}.detections.json"
    if legacy_path.exists():
        logger.info(f"✓ Found legacy rendered detections for {camera_id} (consider re-preprocessing)")
        return str(legacy_path)

    logger.debug(f"No pre-processed detections found for {camera_id} at {detections_path}")
    return None
