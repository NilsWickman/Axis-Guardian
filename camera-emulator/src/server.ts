...
export async function createCameraEmulator(
  ...
  let lastFrameSent = -1
  ffmpegStreamer.on('frame', (frameNumber: number, videoTimeMs: number, rtpTimestamp: number) => {
    ...
    // Include video_time_ms and actual RTP timestamp for frontend sync with video element
    const detectionBuffer = detectionSync.getDetectionForFrame(frameNumber, dispatchTime, videoTimeMs, rtpTimestamp)
    try {
    ...
    // Also send to tracking service with same dispatch time and RTP timestamp
    const rawFrame = detectionSync.getRawDetectionForFrame(frameNumber)
    if (rawFrame) {
      trackingClient.postDetections({ ...rawFrame, dispatch_time: dispatchTime, rtp_timestamp: rtpTimestamp })
    }
  })
