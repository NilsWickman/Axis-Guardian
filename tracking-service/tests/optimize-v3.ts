/**
 * Optimization v3 - Match the exact projection pipeline
 * Uses getBBoxBottomCenter with seated extension to match actual behavior
 */
...
async function main() {
  ...
  // Get camera params for seated extension
  ...
  // Prepare training data using EXACT same process as projection
  ...
  for (const ann of certainAnnotations) {
    for (const det of ann.linkedDetections) {
      // Convert bbox to format expected by getBBoxBottomCenter
      const bbox = {
        x: det.bbox.left,
        y: det.bbox.top,
      ...
      // Use EXACT same method as projectDetectionWithKRT
      const feetPos = getBBoxBottomCenter(
        ...
        1920,
        1080,
        true // applySeatedExtension - matches projectDetectionWithKRT line 797
      )
