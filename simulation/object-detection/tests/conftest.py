"""Pytest configuration and shared fixtures for object detection tests."""
...
def test_config_env(monkeypatch) -> None:
    ...
    monkeypatch.setenv("MQTT_HOST", "test-mqtt-host")
...
def mock_mqtt_client(mocker):
    ...
    mock_client.connect = mocker.Mock(return_value=0)
    mock_client.publish = mocker.Mock(return_value=(0, 1))
    mock_client.loop_start = mocker.Mock()
    mock_client.loop_stop = mocker.Mock()
    mock_client.disconnect = mocker.Mock()
    return mock_client
