...
# Note: This is the legacy native binary mode. For Docker mode, use: make infrastructure
...
# Check if Docker-based MediaMTX is already running
if command -v docker &> /dev/null; then
    if docker ps --format '{{.Names}}' | grep -q '^axis-guardian-mediamtx$'; then
        echo "MediaMTX is already running in Docker (axis-guardian-mediamtx)"
        echo "To stop it, run: docker compose -f docker-compose.dev.yml stop mediamtx"
        exit 0
    fi
fi
