/**
 * Try degree 5 with lambda=1e-3 coefficients
 */
...
async function main() {
  ...
  console.log(`Fitting with lambda=1e-3...`)

  const cam1Result = fitPolynomialRobust(cam1Data, 5, 30, 0.5, 1e-3)
  const cam2Result = fitPolynomialRobust(cam2Data, 5, 30, 0.5, 1e-3)

  // Evaluate with smart selection
  let passed = 0
  let total = 0
  let totalError = 0
  let converged = 0
  ...
  for (const ann of certainAnnotations) {
    ...
    for (const det of ann.linkedDetections) {
      ...
      const transformed = applyPolynomial(rawKRT.x, rawKRT.y, coeffs.coeffsX, coeffs.coeffsY, 5)

      projections[det.cameraId] = transformed
    }

    if (Object.keys(projections).length === 0) continue
    total++

    let finalPosition: Point2D
    if (Object.keys(projections).length === 1) {
      finalPosition = Object.values(projections)[0]
    } else {
      // Smart selection
      const dist = distance(projections['camera1']!, projections['camera2']!)
      if (dist > 0.6) {
        // Divergent - pick camera1
        finalPosition = projections['camera1']!
        multiCameraCount++
        converged += dist <= 0.6 ? 1 : 0
      } else {
  ...
  console.log(`\nWith lambda=1e-3 and smart selection:`)
  console.log(`  Accuracy: ${(passed / total * 100).toFixed(1)}% (${passed}/${total})`)
  console.log(`  Avg Error: ${(totalError / total).toFixed(3)}m`)
  console.log(`  Convergence: ${(converged / multiCameraCount * 100).toFixed(1)}%`)

  if (passed / total > 0.777) {
    console.log('\n=== NEW BEST COEFFICIENTS ===\n')
    const formatCoeffs = (coeffs: number[]) => coeffs.map(c => c.toFixed(8)).join(', ')
    console.log(`CAMERA1_coeffsX: [${formatCoeffs(cam1Result.coeffsX)}]`)
    console.log(`CAMERA1_coeffsY: [${formatCoeffs(cam1Result.coeffsY)}]`)
    console.log(`CAMERA2_coeffsX: [${formatCoeffs(cam2Result.coeffsX)}]`)
    console.log(`CAMERA2_coeffsY: [${formatCoeffs(cam2Result.coeffsY)}]`)
  }
}
