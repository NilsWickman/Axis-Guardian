...
- Organizes outputs by quality level (360p, 480p, 720p, 1080p, 1440p, 4k)
- Optimizes output for low-latency streaming
- Supports batch processing of multiple videos

Usage:
    python preprocess_detections.py --input video.mp4 --output preprocessed.mp4
    python preprocess_detections.py --batch-all  # Process all source videos
    python preprocess_detections.py --list       # Show available videos

Output:
    - {output_name}.mp4          - Pre-processed video with detections
...
try:
    ...
    logger.error("Required packages not found. Install with: pip install ultralytics torch opencv-python supervision")
    sys.exit(1)
...
# Configuration
DEFAULT_MODEL = os.getenv("PREPROCESS_MODEL_PATH", os.getenv("PRERENDER_MODEL_PATH", "shared/models/yolov8x.pt"))  # Backward compat
...
PREPROCESSED_VIDEOS_DIR = PROJECT_ROOT / "shared" / "cameras" / "preprocessed"
MODELS_DIR = PROJECT_ROOT / "shared" / "models"

# Quality presets (resolution and bitrate)
QUALITY_PRESETS = {
    '360p': {'width': 640, 'height': 360, 'bitrate': '500K'},
    '480p': {'width': 854, 'height': 480, 'bitrate': '1M'},
    '720p': {'width': 1280, 'height': 720, 'bitrate': '2M'},
    '1080p': {'width': 1920, 'height': 1080, 'bitrate': '4M'},
    '1440p': {'width': 2560, 'height': 1440, 'bitrate': '6M'},
    '4k': {'width': 3840, 'height': 2160, 'bitrate': '12M'},
}
...
class DetectionPreprocessor:
    """Handles pre-processing of detection videos."""
    ...
    def __init__(
        ...
        Args:
            ...
            confidence: Detection confidence threshold (legacy, use detection_threshold)
            iou: NMS IOU threshold
            inference_size: Size for YOLO inference (width in pixels)
            enable_tracking: Enable object tracking for persistent IDs
            two_tier_confidence: Enable two-tier confidence filtering
            detection_threshold: YOLO output threshold for capturing potential matches
            new_track_threshold: Minimum confidence to start new tracks
        """
        ...
        self.two_tier_confidence = two_tier_confidence and enable_tracking
        self.detection_threshold = detection_threshold
        self.new_track_threshold = new_track_threshold

        logger.info(f"Model loaded successfully")
        if self.two_tier_confidence:
            logger.info(f"Two-tier confidence: detection={detection_threshold}, new_track={new_track_threshold}")
        else:
            logger.info(f"Single-tier confidence threshold: {confidence}")
        ...
        if self.enable_tracking:
            self.tracker = ObjectTracker(
                track_activation_threshold=detection_threshold,  # Match YOLO threshold
                lost_track_buffer=30,
                minimum_matching_threshold=0.7,
                frame_rate=30,  # Assume 30 FPS for pre-processing
                confidence_boost_enabled=True,
                max_confidence_boost=0.15,
            )
            logger.info("Object tracking enabled with ByteTrack and confidence boosting")
    ...
    def detect_frame(
        ...
        # Run YOLO inference
        # Use lower threshold when two-tier confidence is enabled
        conf_threshold = self.detection_threshold if self.two_tier_confidence else self.confidence
        ...
        if self.tracker is not None:
            ...
            # Apply two-tier confidence filtering
            if self.two_tier_confidence:
                detections = self._apply_two_tier_filter(detections)

            # Update pixel coordinates for tracked detections (bboxes may have been smoothed)
    ...
    def _apply_two_tier_filter(self, detections: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        High-confidence detections (>= new_track_threshold) always pass.
        ...
        for det in detections:
            ...
            if confidence >= self.new_track_threshold:
                filtered.append(det)
            # Low confidence - only include if part of existing track
            elif track_state in ['active', 'lost']:
                filtered.append(det)
            # Low confidence new track - exclude (prevents noise from creating tracks)
            else:
                pass
    ...
    def draw_detections(
        ...
        for det in detections:
            ...
            # Prepare label with track ID if available
            if 'track_id' in det:
                label = f"{class_name} #{det['track_id']} {int(confidence * 100)}%"
            else:
    ...
    def _save_enhanced_metadata(
        ...
        # Clean detections - remove internal fields before saving
        for frame in all_detections:
            for det in frame['detections']:
                # Remove internal pixel coordinates (used only for drawing)
                det.pop('_pixel_coords', None)
                # Keep track_id and track_state as they're part of the public API
    ...
    def process_video(
        ...
        if output_resolution:
            # Use QUALITY_PRESETS for resolution
            if output_resolution in QUALITY_PRESETS:
                preset = QUALITY_PRESETS[output_resolution]
        ...
        # Setup direct H.264 encoding via FFmpeg pipe (optimized - no temp file)
        # This is much faster than XVID → H.264 two-step process
        import subprocess

        logger.info(f"Using direct H.264 encoding via FFmpeg pipe (optimized)")
        ...
        try:
            ...
            logger.error("FFmpeg not found! Falling back to two-step encoding...")
            # Fallback to old two-step method
        ...
        while True:
            ...
            # Write frame to FFmpeg pipe or VideoWriter
            if ffmpeg_process is not None:
                try:
                    ffmpeg_process.stdin.write(frame.tobytes())
                except BrokenPipeError:
                    logger.error("FFmpeg pipe broken! Check FFmpeg stderr")
                    stderr = ffmpeg_process.stderr.read().decode('utf-8')
        ...
        if ffmpeg_process is not None:
            ...
            if ffmpeg_process.returncode != 0:
                ...
                raise ValueError(f"FFmpeg encoding failed with return code {ffmpeg_process.returncode}")

            logger.info(f"✓ Direct H.264 encoding complete!")
            logger.info(f"  Final output: {output_path}")
        else:
            # Two-step fallback: convert XVID to H.264
            out.release()
            logger.info(f"Converting XVID to H.264 with FFmpeg...")
...
def list_source_videos():
    ...
    # Exclude already preprocessed videos and any videos in subdirectories
    videos = [v for v in videos if v.parent == SOURCE_VIDEOS_DIR and "preprocessed" not in v.stem and "rendered" not in v.stem]
    return sorted(videos)


def check_if_needs_preprocessing(video_path: Path, quality: str = '720p') -> bool:
    """
    Check if a video needs to be pre-processed at a given quality.

    A video needs pre-processing if:
    - No preprocessed version exists for this quality
    - Preprocessed version is older than source
    - Detection JSON is missing

    Args:
        video_path: Path to source video
        quality: Quality preset (360p, 480p, 720p, 1080p, 1440p, 4k)

    Returns:
        True if video should be (re)preprocessed
    """
    output_name = video_path.stem + "-preprocessed.mp4"
    quality_dir = PREPROCESSED_VIDEOS_DIR / quality
    output_path = quality_dir / output_name
    ...
    # Check if source is newer than preprocessed
    source_mtime = video_path.stat().st_mtime
    preprocessed_mtime = output_path.stat().st_mtime

    if source_mtime > preprocessed_mtime:
        logger.info(f"Source video {video_path.name} is newer than preprocessed version")
        return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Pre-process detection videos for high-performance streaming",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process a single video
  python preprocess_detections.py --input people-detection.mp4 --output people-preprocessed.mp4

  # Process all source videos at 720p quality (recommended)
  python preprocess_detections.py --batch-all --quality 720p

  # List available videos
  python preprocess_detections.py --list

  # Use custom model and thresholds
  python preprocess_detections.py --input video.mp4 --model yolov8m.pt --confidence 0.6

  # Process at different qualities
  python preprocess_detections.py --batch-all --quality 480p   # Low bandwidth
  python preprocess_detections.py --batch-all --quality 1080p  # High quality
  python preprocess_detections.py --batch-all --quality 4k     # Ultra quality
        """
    )
    ...
    parser.add_argument(
        "--output", "-o",
        type=str,
        help="Output video file (saved to shared/cameras/preprocessed/{quality}/)"
    )
    parser.add_argument(
        "--batch-all",
        action="store_true",
    ...
    parser.add_argument(
        "--force", "-f",
        action="store_true",
        help="Force re-preprocessing even if output files already exist"
    )
    parser.add_argument(
        "--quality", "-q",
        type=str,
        choices=['360p', '480p', '720p', '1080p', '1440p', '4k'],
        default=os.getenv('PREPROCESSED_QUALITY', os.getenv('RENDER_RESOLUTION', '720p')),
        help="Output quality preset (default: from PREPROCESSED_QUALITY env var, or 720p). 720p recommended for balance of quality and performance."
    )

    args = parser.parse_args()

    # Show quality being used
    if args.quality:
        logger.info(f"Using quality preset: {args.quality}")
        if os.getenv('PREPROCESSED_QUALITY') and not any('--quality' in arg or '-q' in arg for arg in sys.argv):
            logger.info(f"  (from PREPROCESSED_QUALITY environment variable)")
    else:
    ...
    if args.batch_all:
        ...
        for video in videos:
            if args.force or check_if_needs_preprocessing(video, args.quality):
                videos_to_process.append(video)
            else:
                videos_skipped.append(video)

        if videos_skipped:
            logger.info(f"\nSkipping {len(videos_skipped)} videos (already preprocessed at {args.quality}):")
            for video in videos_skipped:
                logger.info(f"  ✓ {video.name}")
            logger.info("\nUse --force to re-preprocess all videos")

        if not videos_to_process:
            logger.info(f"\n✓ All videos are already pre-processed at {args.quality}!")
            logger.info(f"Preprocessed videos: {PREPROCESSED_VIDEOS_DIR / args.quality}")
            return

        # Create preprocessor for batch processing
        ...
        for i, video in enumerate(videos_to_process, 1):
            ...
            output_name = video.stem + "-preprocessed.mp4"
            quality_dir = PREPROCESSED_VIDEOS_DIR / args.quality
            output_path = quality_dir / output_name

            try:
                preprocessor.process_video(video, output_path, output_resolution=args.quality)
                processed += 1
            except Exception as e:
        ...
        logger.info(f"  Output: {PREPROCESSED_VIDEOS_DIR / args.quality}")
        return
    ...
    if args.output:
        ...
        if not output_path.is_absolute():
            output_path = quality_dir / output_path
    else:
        output_name = input_path.stem + "-preprocessed.mp4"
        output_path = quality_dir / output_name

    # Check if already preprocessed (unless --force)
    if not args.force and not check_if_needs_preprocessing(input_path, args.quality):
        logger.info(f"✓ Video already preprocessed: {output_path}")
        logger.info("Use --force to re-preprocess")
        return

    # Now create preprocessor since we have work to do
    preprocessor = DetectionPreprocessor(
        model_path=args.model,
        confidence=args.confidence,
        iou=args.iou,
    ...
    # Process video
    try:
        preprocessor.process_video(input_path, output_path, output_resolution=args.quality)
        logger.info("\n✓ Success! Video ready for streaming.")
        if args.quality:
            logger.info(f"Preprocessed at {args.quality} for optimized performance")
    except Exception as e:
