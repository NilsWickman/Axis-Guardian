...
- `tracking-service/` — Fastify + TypeScript API (`tracking-service/src/`), tests in `tracking-service/tests/` and `src/**/*.test.ts`, SQLite DB in `tracking-service/data/`.
...
- `make dev` — start all services (frontend `5173`, camera emulator `9101`, tracking service `3010`).
- `make dev-frontend | dev-camera | dev-tracking` — start a single service.
...
- `cd tracking-service && pnpm dev | build | test | test:coverage | db:seed`
...
## Testing Guidelines
- Test runner: Vitest (`frontend/`, `tracking-service/`).
- Keep tests deterministic and focused; name files `*.test.ts`.
- Integration tests live in `tracking-service/tests/integration/`.
