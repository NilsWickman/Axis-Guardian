/**
 * Optimization v4 - Push for higher accuracy
 * Try:
 * 1. Higher IRLS iterations
 * 2. Different robust weighting (Tukey bisquare, Cauchy)
...
function fitPolynomialRobust(
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    const mad = medianResidual * 1.4826

    const c = Math.max(threshold, mad * 2)

    for (let i = 0; i < data.length; i++) {
      const r = residuals[i]
      if (weightFunc === 'tukey') {
        weights[i] = tukeyWeight(r, c)
      } else if (weightFunc === 'cauchy') {
        weights[i] = cauchyWeight(r, c)
      } else {
        // Huber-like
        if (r < threshold) {
          weights[i] = 1.0
...
async function main() {
  ...
  const configs = [
    { name: 'Huber 20 iters', weightFunc: 'huber' as const, iters: 20, threshold: 0.5 },
    { name: 'Huber 50 iters', weightFunc: 'huber' as const, iters: 50, threshold: 0.5 },
    { name: 'Tukey 20 iters', weightFunc: 'tukey' as const, iters: 20, threshold: 0.5 },
    { name: 'Cauchy 20 iters', weightFunc: 'cauchy' as const, iters: 20, threshold: 0.5 },
    { name: 'Huber thresh=0.4', weightFunc: 'huber' as const, iters: 20, threshold: 0.4 },
    { name: 'Huber thresh=0.6', weightFunc: 'huber' as const, iters: 20, threshold: 0.6 },
  ]
  ...
  for (const config of configs) {
    ...
    const cam1Result = fitPolynomialRobust(cam1Data, 4, config.iters, config.weightFunc, config.threshold)
    const cam2Result = fitPolynomialRobust(cam2Data, 4, config.iters, config.weightFunc, config.threshold)
    ...
    for (const ann of certainAnnotations) {
      ...
      for (const det of ann.linkedDetections) {
        ...
        const transformed = applyPolynomial(rawKRT.x, rawKRT.y, coeffs.coeffsX, coeffs.coeffsY, 4)
